...
function systemSection(){
  ...
  AppStorage.fetch('/api/system').then(async r=>{const s=await r.json();if(!r.ok)throw new Error(s.error||'Server unavailable');state.textContent=AppStorage.mode==='server'?'Self-hosted server. HTTP Auth is '+(s.authEnabled?'enabled.':'disabled.'):'Static / standalone mode. Calculations and plan storage stay in this browser. Export JSON to back up or move your plan.';}).catch(error=>state.textContent=error.message);
  const auth=document.createElement('div');auth.innerHTML='<h3>HTTP Auth on Unraid</h3><p class="inline-help">Edit the container’s environment variables, then restart it. Authentication protects both the pages and real-time sync. Credentials stay on the server.</p><pre>BASIC_AUTH_ENABLED=true\nBASIC_AUTH_USER=your-username\nBASIC_AUTH_PASS=your-password</pre><p class="inline-help">The Docker port defaults to 3333. Keep the data folder mapped to persistent Unraid storage. Standalone plans can be moved between browsers with Export JSON and Import JSON.</p>';if(AppStorage.deployment==='server')sec.append(auth);return sec;
}
