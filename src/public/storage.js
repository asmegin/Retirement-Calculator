(function(root){
  ...
  root.addEventListener('storage',event=>{if(location.protocol==='file:'&&!demoActive&&deployment==='static'&&event.key===local.stateKey&&event.newValue){try{emit(JSON.parse(event.newValue).config);}catch(_){} }});
