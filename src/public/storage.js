(function(root){
  ...
  const nativeFetch=root.fetch.bind(root);let current,signature,online=deployment==='static',realtime=false,socket,polling;
  ...
  async function remote(path,options={}){
    try{
      ...
      return response;
    }catch(_){setOnline(false);throw new Error('Server unavailable. Your save was not confirmed. Reconnect before saving, or Export JSON to keep your edits.');}
  }
  ...
  function startRealtime(){
    ...
    script.onload=()=>{
      ...
      socket.on('config_updated',async c=>{setOnline(true);await cache(c);emit(c);});
    };
  ...
  function download(c){PlanSchema.validate(c);const a=document.createElement('a'),url=URL.createObjectURL(new Blob([JSON.stringify(c,null,2)],{type:'application/json'}));a.href=url;a.download='retirement-plan.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
  root.AppStorage={ready,fetch:request,save,download,validate:PlanSchema.validate,mode,deployment,get online(){return online;},get realtime(){return realtime;},getPreference:local.getPreference,setPreference:local.setPreference,bindState(get,set){root.PlanState={get,set};},subscribe(fn){listeners.add(fn);return()=>listeners.delete(fn);}};
  root.addEventListener('storage',event=>{if(deployment==='static'&&event.key===local.stateKey&&event.newValue){try{emit(JSON.parse(event.newValue).config);}catch(_){} }});
  local.subscribe(c=>{if(deployment==='static')emit(c);});
})(window);
