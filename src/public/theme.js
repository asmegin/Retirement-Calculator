(function(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    bar.innerHTML='<span class="storage-mode" role="status"></span><button class="btn ghost" id="export-json">Export Plan</button><label class="backup-encryption"><input id="encrypt-backup" type="checkbox"> Encrypt backup with a password</label><button class="btn ghost" id="import-json">Import Plan</button><input type="file" accept=".json,application/json" id="import-json-file" hidden aria-label="Import retirement plan JSON"><span id="file-status" role="status"></span>';
    ...
    bar.querySelector('#export-json').onclick=async()=>{try{await AppStorage.ready;const done=await PlanBackupUI.export(window.PlanState?.get()||(await(await AppStorage.fetch('/api/config')).json()),bar.querySelector('#encrypt-backup').checked);status.textContent=done?'Plan exported.':'Export cancelled.';}catch(error){status.textContent=error.message;}};
    const input=bar.querySelector('#import-json-file');bar.querySelector('#import-json').onclick=()=>input.click();
    input.onchange=async()=>{try{if(!input.files[0])return;const imported=await PlanBackupUI.read(input.files[0]);if(!imported){status.textContent='Import cancelled.';return;}const c=await AppStorage.save(imported);window.PlanState?.set(c);status.textContent='Plan imported and recalculated.';}catch(error){status.textContent=error.message;}finally{input.value='';}};
