(function(){
  ...
  function mountPlanFileTools(container,status){
    ...
    input.onchange=async()=>{try{if(!input.files[0])return;const imported=await PlanBackupUI.read(input.files[0]);if(!imported){report('Import cancelled.');return;}const c=await AppStorage.save(imported);window.PlanState?.set(c);report('Plan imported and recalculated.');}catch(error){report(error.message);}finally{input.value='';}};
