...
(function(root){
  ...
  root.runRetirementTask=function(kind,data){
    ...
    try{
      let result;
      if(kind==='comparison'){
        const options={propertyIndex:data.propertyIndex,propertyIndices:data.propertyIndices,compareCCA:data.compareCCA,maxEvaluations:data.maxEvaluations,searchMode:data.searchMode,onProgress:progress};
