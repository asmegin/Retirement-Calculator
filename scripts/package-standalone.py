...
with ZipFile(output,'w',ZIP_DEFLATED) as bundle:
    for item in sorted(public.rglob('*')):
        if item.is_file() and item.suffix.lower() in {'.html','.js','.css','.svg','.png','.ico','.txt','.woff2','.webmanifest'}:
